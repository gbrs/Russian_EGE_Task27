...
''''
На каждом шаге достаём из массива один элемент, 
...
# создаем Массив из 5 элементов (пока пустой)
s = 5
a = [0] * s

n = int(input())  # общее количество чисел

# вводим 5 первых элементов
for i in range(s):
    a[i] = int(input())
...
for i in range(s, n):
    ...
    # убирая текущий элемент из Массива проверяем его на кратность 13
    if a[k] % 13 == 0:
        n13 += 1

    a_ = int(input())  # a_ - обозначение для нововводимого числа

    # если число кратно 13, то общее число пар увеличивается на количество чисел за вычетом четырех последних
    if a_ % 13 == 0:
        count = count + (i + 1) - s
